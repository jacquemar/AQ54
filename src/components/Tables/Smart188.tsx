...
const SMART188: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('http://localhost:3000/stations/current-month?stations=SMART188');
      if (!response.ok) {
  ...
  return (
    <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      ...
      <h2 className="text-2xl font-bold text-center text-black dark:text-white mb-6">
        DONNÉES MENSUEL DU CAPTEUR SMART188 
      </h2>
